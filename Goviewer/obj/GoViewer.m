classdef GoViewer < handle
  ...
  methods(Static)
    ...
    function ufig = CreateCommentWindow(fig)
      ...
      textarea.ValueChangedFcn=@CallbackSet.CommentValueChangedCallback;
    ...
    function enableJavaFigureInteractivity(fig)
      % Get the undocumented java figure component object
      % WARN: The undocumented JavaFrame property will be removed, and it
      % is not safe to handle exposed-JAVA object directly.
      
      msg1='MATLAB:HandleGraphics:ObsoletedProperty:JavaFrame';
      msg2='MATLAB:ui:javaframe:PropertyToBeRemoved';
      warning('off',msg1);
      warning('off',msg2);
      o1 = onCleanup(@() warning('on',msg1));
      o2 = onCleanup(@() warning('on',msg2));
      
      try
        figFrame=get(handle(fig),'JavaFrame'); %#ok
        %Client = figFrame.fHG2Client;
        component=figFrame.fHG2Client.getFigurePanelContainer;
        %component=Client.getAxisComponent;
      catch
        component=[];
      end
...
function setJavaCallback(component,prop,funhandle,varargin)
% set the java callback properties

handleObj=handle(component.getComponent(0),'CallbackProperties');
setappdata(handleObj,'Figure',getappdata(component,'Figure'));
if(nargin==3)
  set(handleObj, prop, funhandle);
elseif(nargin>3)
  set(handleObj, prop, {funhandle,varargin{:}}); %#ok
end
